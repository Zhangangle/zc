...
public class ChatListView extends ListView implements OnScrollListener {
	...
	private IChatListViewListener mListViewListener;
	...
	// -- footer view
	private ChatListViewFooter mFooterView = null;
	...
	private void initWithContext(Context context) {
		...
		mHeaderViewContent = (RelativeLayout) mHeaderView
				.findViewById(R.id.relayout_header_content);
	...
	public void pullRefreshing() {
		...
		mHeaderView.setState(ChatListViewHeader.STATE_REFRESHING);
	}
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			//mPullLoading = false;
			if (mFooterView == null) {
				mFooterView = new ChatListViewFooter(m_context);
			...
			mFooterView.setState(ChatListViewFooter.STATE_NORMAL);
	...
	private void updateHeaderHeight(float delta) {
		...
		if (mEnablePullRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
			if (mHeaderView.getVisiableHeight() > mHeaderViewHeight) {
				mHeaderView.setState(ChatListViewHeader.STATE_READY);
			} else {
				mHeaderView.setState(ChatListViewHeader.STATE_NORMAL);
			}
		}
	...
	private void updateFooterHeight(float delta) {
		...
		if (mEnablePullLoad && !mPullLoading) {
			if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke load
													// more.
				mFooterView.setState(ChatListViewFooter.STATE_READY);
			} else {
				mFooterView.setState(ChatListViewFooter.STATE_NORMAL);
			}
		}
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(ChatListViewFooter.STATE_LOADING);
	...
	public void setChatListViewListener(IChatListViewListener l) {
		mListViewListener = l;
	}
	 ...
	 */
	public interface IChatListViewListener {
